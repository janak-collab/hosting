...
# Fix script to remove MRN field from dictation form
# Run from SSH in the home directory

echo "=== Removing MRN field from Medical Dictation Form ==="
echo

# Navigate to the public_html directory
cd ~/public_html

# Create a backup of the current dictation.php
echo "1. Creating backup of dictation.php..."
cp dictation.php dictation.php.backup.$(date +%Y%m%d_%H%M%S)

# Update the dictation.php file to remove MRN references
echo "2. Updating dictation.php to remove MRN field..."
...
if grep -q "MRN" dictation.php; then
    echo "⚠️  Warning: MRN field references still found in the file"
    echo "   Manual review may be needed"
else
    echo "✅ MRN field successfully removed from dictation.php"
fi

echo
echo "=== Update Complete ==="
echo
echo "The MRN field has been removed from:"
echo "- Patient information form"
echo "- Dictation output"
echo "- All form validation"
echo
echo "Test the updated form at: https://gmpm.us/dictation"
echo
echo "Backup saved as: dictation.php.backup.$(date +%Y%m%d_%H%M%S)"
